...
def plot_correlator(corr, title, log_plot):
    filename = f"../observables/{corr}.csv"
    ...
    if log_plot:
        heatmap = ax0.imshow(np.log(average_values), cmap='viridis', aspect='equal')
        contours = ax0.contour(np.log(average_values), colors='black', linewidths=0.5)
    else:
        heatmap = ax0.imshow((average_values), cmap='viridis', aspect='equal')
        contours = ax0.contour((average_values), colors='black', linewidths=0.5)
    ...
    ax2.errorbar(np.arange(L), average_summed_values_1, std_summed_values_1)
    if log_plot:
        ax2.set_yscale('log')
    ax2.set_xlabel('t')
    
    fig.savefig(f"../plots/{corr}.png")
