...
ax1.set_ylabel(r"$(x/a)\cdot\sum_t \langle P(t, x) P(0, 0) \rangle$")

ax2.plot(m*np.arange(T)[1:T//2], (summed_over_1_avg)[1:T//2]*(np.arange(T)[1:T//2]), color='black', marker='s', label=r'lattice measurement ($L_0 = %d$, $L = %d$)'%(T, L))
ax2.set_xlabel(r"$t \cdot m = (t/a) \cdot (m\cdot a)$")
ax2.set_ylabel(r"$(t/a)\cdot\sum_x \langle P(t, x) P(0, 0) \rangle$")

def integrand(k, xm):
    return 1/(np.pi)*np.exp(-2*xm*np.sqrt(1+(k/xm)**2))
...
ax2.plot(tm_values, scale*(integral_values), label=f"continuum infinite volume result")
...
fig.tight_layout()
fig.savefig(f"{plot_folder}pseudoscalar.png")
plt.close(fig)
